...
Encoding in baselines & Same $\Theta$ discretisation imposed on NSGA-II/NSGANetV2 (they forgo their usual real-valued crossover); NSGANetV2 additionally run once more in its native continuous encoding as a control & Discretisation is required for the ``identical genotype encoding'' fairness rule, but is not neutral: it costs NSGA-II/NSGANetV2 their native crossover while costing P3 nothing; the continuous-$\Theta$ control run bounds this asymmetry empirically instead of only asserting fairness by construction & Results, Fairness controls \\
\bottomrule
\end{longtable}
...
Mechanism lineage & Analogous to eLyMPuS (the empirical variant of LyMPuS, built on an incompletely discovered dependency structure) & NAS validation accuracy does not decompose into subfunctions (no gray-box access) --- eLyMPuS matches this black-box regime & Related Work \\
...
Baselines & NSGANetV2 (primary, discretised $\Theta$); NSGANetV2 in its native continuous $\Theta$ (control, isolating the effect of discretisation itself); NSGA-Net with no surrogate, every candidate fully evaluated (engine-only ablation, NSGA-II side, symmetric to the P3 row below); P3 without a surrogate (engine-only ablation, P3 side); P3 with an absolute regressor in the NSGANetV2 style (surrogate-only ablation); random search and TPE (sanity checks) & Attributes gains to search engine vs.\ surrogate vs.\ discretisation as far as the design permits; not a full orthogonal factorial, since the relative surrogate $\hat\delta_F$ is only defined given a linkage tree, so an NSGA-II-plus-relative-surrogate cell does not exist & Results \\
...
Metrics & Fixed-budget hypervolume always; IGD+ only where the oracle front is enumerable (i.e.\ NAS-HPO-Bench-II only) & Metric matched to what each benchmark actually offers & Results \\
...
Comparison set for correction & P3Net vs.\ each of the other seven arms, per benchmark and per budget tier --- not every pairwise combination among all arms & Makes explicit what Holm--Bonferroni is correcting across, since the corrected significance threshold depends on this count & Results, Statistical plan \\
...
Statistical plan: ``full set of comparisons'' undefined & Fixed & Enumerated: P3Net vs.\ each of the other seven arms, per benchmark and budget tier & Results, Statistical plan \\
...
``in the spirit of eLyMPuS'' phrase repeated across three sections & Rejected & Each instance is a short local gloss with a forward pointer, not a verbatim block copy; removing any one would reintroduce the forward-reference gap it was added to close & Introduction; Proposed Optimizer \\
