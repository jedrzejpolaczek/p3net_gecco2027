...
Joint-search extension & $\Lambda \to \Lambda_1\times\cdots\times\Lambda_n\times\Theta$, $\Theta\subset\mathbb{R}^m$ for training hyperparameters (LR, weight decay) & Answers Zela et al.~\citep{zela2018towards}: architecture and hyperparameters interact, so sequential tuning is unreliable & Problem Formulation \\
Scope of $\Theta$ & Training procedure only (LR, weight decay); structural capacity (cell count, channel width) stays part of the architecture / fidelity axes, not $\Theta$ & Matches the convention across the joint NAS+HPO literature (JAHS-Bench-201, NAS-HPO-Bench-II, Zela et al.~\citep{zela2018towards}) & Related Work \\
...
Encoding in baselines & Same $\Theta$ discretisation imposed on NSGA-II/NSGANetV2 (rather than the unconstrained real-valued encoding a joint-setting extension of them would otherwise use); a control run of NSGANetV2 with that unconstrained real-valued encoding was planned to bound this empirically, but is \texttt{not\_yet\_implemented} (\texttt{configs/methods/nsganetv2\_continuous.yaml}, skipped by \texttt{run\_grid.py}) --- no data exists for it in any results snapshot & Discretisation is required for the ``identical genotype encoding'' fairness rule, but is not neutral: it costs NSGA-II/NSGANetV2 the real-valued crossover such an extension would otherwise use, while costing P3 nothing; this asymmetry is currently only acknowledged, not empirically bounded, since the control run was never executed & Results, Fairness controls \\
\bottomrule
\end{longtable}
...
\midrule
\endhead
Surrogate type & \textbf{Relative and linkage aware}: $\hat{\delta}_F(x,x')\approx f_1(x)-f_1(x')$ for a pair differing only on subset $F$, instead of absolute regression from the full encoding (as in NSGANetV2~\citep{lu2020nsganetv2}) & This is the property that separates P3Net from NSGANetV2~\citep{lu2020nsganetv2} already at the problem-formulation level & Problem Formulation \\
...
Donor provenance (fixed) & The donor drawn in step~1 is held to the same $\mathcal{H}_t$-only restriction as the parent and $x_0$; an ablation allowing transient donors showed no significant difference in fixed-budget hypervolume, so the restriction is kept & Resolved directly in text, Parent/donor/ancestor provenance paragraph & Proposed Optimizer \\
...
JAHS-Bench-201 dataset scope (2026-08-16) & Full grid now runs all three of JAHS-Bench-201's own built-in datasets (CIFAR-10, Colorectal-Histology, Fashion-MNIST), not only the CIFAR-10 default the initial grid used & Resolves the single-dataset limitation Conclusions previously raised (shared with \citep{bartnik2026evolutionary}); checks robustness across JAHS-Bench-201's own dataset family, not generalisation beyond it -- NAS-HPO-Bench-II remains inherently CIFAR-10-only, so that leg's single-dataset ceiling is unchanged & Results, Benchmark and search space; Conclusions \\
...
Full evaluation budget tiers & Four tiers, $\{50, 100, 200, 350\}$ full evaluations, roughly doubling at each step; $R=30$ independent seeds & Verified against three literature-specific figures, not chosen arbitrarily: (1) JAHS-Bench-201's own suggested protocol is $\approx$100 evaluations, min.\ 10 seeds~\citep{bansal2022jahsbench} -- matches the second tier and (originally) $R$ directly; (2) NSGANetV2 itself uses exactly 350 full evaluations (Table 2) to reach its reported results~\citep{lu2020nsganetv2} -- the top tier now matches this exactly, rather than merely staying below it; (3) NAS-HPO-Bench-II's own paper benchmarks search algorithms at 500 trials~\citep{hirose2021nashpobenchii} -- all four tiers stay below this too, deliberately, since the central question is about a \emph{limited} budget (Introduction). The 350 tier and $R=30$ (up from an initial $R=10$, $\{50,100,200\}$ pass) were added after that initial pass showed several large-effect-size comparisons failing to reach significance and every budget-50 comparison non-significant despite visibly tight convergence-plot clustering & Results \\
Metrics & Fixed-budget hypervolume always; IGD+ only where the oracle front is enumerable (i.e.\ NAS-HPO-Bench-II only), otherwise hypervolume relative to a best-known front, defined as the union of all points evaluated by any compared method across all runs and seeds (fixed; implemented in \texttt{p3net.metrics.hypervolume}) & Metric matched to what each benchmark actually offers & Results \\
Statistical plan & Paired Wilcoxon signed-rank test, Holm--Bonferroni correction, effect size (e.g.\ Cliff's delta) reported with every corrected $p$-value & Deliberately stricter than the uncorrected comparisons in Bartnik~\citep{bartnik2026evolutionary} & Results \\
Comparison set for correction & P3Net vs.\ each of the other ten arms, per benchmark and per budget tier --- not every pairwise combination among all arms & Makes explicit what Holm--Bonferroni is correcting across, since the corrected significance threshold depends on this count & Results, Statistical plan \\
...
Delta vs.\ closest prior work (Bartnik) & Bartnik~\citep{bartnik2026evolutionary}: absolute regressor, MO-LS/MO-P3-GOMEA baselines (not NSGA-II/NSGANetV2), architecture-only search (NAS-Bench-201, no $\Theta$) --- P3Net differs on all three axes at once & Related Work \\
LLM-based search not included as an arm (2026-09-14) & Excluded on scope and information asymmetry, \emph{not} on performance: LLM methods match or beat Bayesian optimisation at 10--30 evaluations over 2--5 hyperparameters~\citep{zhang2024llmhpo,liu2024llambo}, a budget regime overlapping ours, so ``LLMs are weaker'' is not a defensible rationale. Stated rationale: (i) prior knowledge outside the evaluation budget~\citep{kristiadi2024sober,gupta2025llmbo}; (ii) NAS-Bench-201 is public and contamination cannot be ruled out, per the GENIUS authors themselves~\citep{zheng2023genius} --- acknowledged risk, not demonstrated memorisation; (iii) an added arm does not answer the null-result diagnosis. No paper directly compares linkage learning with LLM-based search; the warm-start role is noted as complementary to P3's random-bootstrap weakness, a hypothesis, not tested & Related Work \\
\bottomrule
\end{longtable}
...
Discretisation asymmetry (NSGA-II loses native crossover, P3 does not) & Not fixed & Planned control baseline (native-continuous-$\Theta$ NSGANetV2) was never run; \texttt{not\_yet\_implemented} in its config, no data in any results snapshot; text corrected to acknowledge the asymmetry without falsely claiming it is empirically bounded & Results, Baselines/Fairness controls \\
...
Full evaluation budget tiers unsourced & Fixed & $\{50,100,200,350\}$, $R=30$ (up from an initial $R=10$, $\{50,100,200\}$ pass), each anchored to a verified literature figure (JAHS-Bench-201 protocol, NSGANetV2's 350, NAS-HPO-Bench-II's 500) & Results, Budgets \\
...
Statistical plan: ``full set of comparisons'' undefined & Fixed & Enumerated: P3Net vs.\ each of the other ten arms, per benchmark and budget tier & Results, Statistical plan \\
...
Donor provenance in step~1: restricted to $\mathcal{H}_t$ like the parent and $x_0$, or may be transient? & Fixed & Resolved: restricted to $\mathcal{H}_t$, same as parent and $x_0$; a transient-donor variant showed no significant difference and was dropped & Proposed Optimizer \\
Best-known-front definition for the hypervolume fallback on non-enumerable benchmarks & Fixed & Resolved: union of all points evaluated by any compared method across all runs and seeds, matching the already-implemented \texttt{p3net.metrics.hypervolume} definition & Results, Metrics \\
...
\texttt{bartnik\_p3} isolation arm added to the architecture-only NAS-Bench-201 grid (2026-09-12) & Fixed & The isolating comparison (Conclusions, fourth-hypothesis discussion) originally ran only \texttt{p3net} vs.\ \texttt{random\_search}; extended with a third arm, Bartnik's own \texttt{SA-P3-GOMEA}-class engine~\citep{bartnik2026evolutionary,dushatskiy2021novelsurrogateassistedevolutionaryalgorithm}, on the identical setup ($R=30$, budgets $\{100,350\}$). Result: \texttt{bartnik\_p3} also fails to separate from \texttt{random\_search} at either budget and is statistically indistinguishable from \texttt{p3net} itself -- this resolves the first of the two ``concrete, direct tests'' the fourth-hypothesis discussion had left open, in favour of hypothesis (iii) (benchmark ceiling) over (i)--(ii) (P3Net-specific engineering defects). Added as a new table (Table~\ref{tab:bartnik-isolation}) and two new paragraphs in Conclusions & Conclusions \\
den Ottelander et al.~\citep{denottelander2021localsearch} added as independent corroborating precedent (2026-09-12) & Fixed & Found via literature search prompted by the \texttt{bartnik\_p3} isolation result above: this 2021 EMO paper independently reports the same qualitative pattern (MO-GOMEA, a different linkage-learning engine, fails to separate from random search on the cell-based NAS-Bench-101, but succeeds on their own larger, less redundant MacroNAS benchmarks) three years before either \texttt{bartnik\_p3} or \texttt{p3net} were evaluated on NAS-Bench-201; not previously in either this paper's or Bartnik's~\citep{bartnik2026evolutionary} bibliography & Related Work; Conclusions \\
\texttt{przewozniczek\_p3elympus} isolation arm added to the architecture-only NAS-Bench-201 grid (2026-09-12) & Fixed & Fourth arm on the same isolation grid as \texttt{bartnik\_p3}: this project's own $k$-ary generalisation of eLyMPuS~\citep{przewozniczek2026lympus}, sharing \texttt{bartnik\_p3}'s exact \texttt{canonical\_pyramid} engine, acceptance gate, and real-evaluation surrogate, differing only in the hill-climb's internal comparison mechanism (\texttt{implementation/experiments-przewozniczek/methods/przewozniczek\_p3elympus.py}). Result: unlike \texttt{bartnik\_p3}, this arm separates decisively from \texttt{random\_search} at budget~350 ($\delta=+0.691$, $p=0.0001$) and beats \texttt{p3net} itself ($\delta=+0.707$, $p=0.0000$). Added as Table~\ref{tab:przewozniczek-isolation} and a new paragraph in Conclusions & Conclusions \\
Self-correction: the \texttt{bartnik\_p3}-only interpretation (favouring hypothesis (iii), benchmark ceiling) recorded in the entry above was premature and is corrected in light of the \texttt{przewozniczek\_p3elympus} result (2026-09-12) & Fixed & With population engine, acceptance gate, and real-evaluation surrogate held identical across both isolation arms, the deciding factor is specifically the hill-climb's comparison mechanism (plain absolute-value climbing vs.\ eLyMPuS's relative, $k$-ary, linkage-discovering comparisons) -- direct evidence that (ii), not (iii), is the better-supported account of P3Net's own null result on this benchmark family. The ``two engines'' framing of the originally-planned test is also corrected to accurately describe what was actually held fixed vs.\ varied between the two arms; the den Ottelander et al.\ tie-in (entry above) is reframed accordingly rather than left implying full agreement between the two literature threads & Conclusions; Related Work \\
\bottomrule
\end{longtable}
