...
def test_calibration_r2_is_perfect_for_exact_predictions():
    assert calibration_r2([1.0, 2.0, 3.0, 4.0], [1.0, 2.0, 3.0, 4.0]) == pytest.approx(1.0)


def test_calibration_r2_is_zero_for_always_predicting_the_mean():
    true_deltas = [1.0, 2.0, 3.0, 4.0]
    mean = sum(true_deltas) / len(true_deltas)
    assert calibration_r2([mean] * 4, true_deltas) == pytest.approx(0.0)


def test_calibration_r2_is_negative_for_worse_than_mean_predictions():
    # Predictions anti-correlated with the truth -- worse than just
    # guessing the mean every time, so R^2 must go negative (unlike
    # pairwise_comparison_accuracy, which is bounded in [0, 1]).
    value = calibration_r2([4.0, 3.0, 2.0, 1.0], [1.0, 2.0, 3.0, 4.0])
    assert value < 0.0


def test_calibration_r2_rejects_mismatched_lengths():
    with pytest.raises(ValueError):
        calibration_r2([1.0, 2.0], [1.0])


def test_calibration_r2_rejects_empty_input():
    with pytest.raises(ValueError):
        calibration_r2([], [])


def test_calibration_r2_rejects_zero_variance_truth():
    """R^2 is undefined (0/0) when every true value is identical -- raise
    rather than silently return a meaningless number, matching
    rank_correlation's "needs enough real variation" precedent."""
    with pytest.raises(ValueError):
        calibration_r2([1.0, 2.0, 3.0], [5.0, 5.0, 5.0])
