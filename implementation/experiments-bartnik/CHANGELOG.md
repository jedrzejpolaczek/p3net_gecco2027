...
everything below is specific to this package.

### Added (Bartnik-algorithm-class isolation experiment -- Fazy 0-4 of notes/plans/experiments-bartnik-plan.md, 2026-08-19)

Direct diagnostic test of hypotheses (i)-(ii) from `conclusions/main.tex` ("Why the results are what they
are"): reconstructs the algorithm class Bartnik (`bartnik2026evolutionary`) showed separating from baselines
on architecture-only NAS-Bench-201, run inside this same harness against the SAME baseline set P3Net itself is
compared against (`random_search`, `sh_emoa`, `tpe`, `mo_bohb`, `nsga_net`, `nsganetv2`, `p3_alone`,
`p3_absolute`, plus `p3net` itself as a direct engine-vs-engine reference point) -- not her own
MO-LS/MO-P3-GOMEA baselines. New code, verified but **not yet run against real data at scale** -- the full
grid is a manual step (below); a real-data smoke test was run.

**Packaging decision (plan's Faza 1 "do ustalenia"), revised**: this work was first added directly into
`implementation/experiments`, then moved out into this separate `experiments-bartnik` sibling package (a full
fork, not a dependency on `experiments/`) so it stays an independently modifiable copy of the harness rather
than accumulating one-off methods in the main P3Net comparison package. `implementation/experiments` was
reverted to its pre-fork state; this package now owns `methods/bartnik_p3.py`,
`configs/methods/bartnik_p3.yaml`, `scripts/run_bartnik_isolation.py`, and `tests/test_bartnik_p3.py` alone.

**Faza 1**: `p3net.search_engines.p3.canonical_pyramid` (library) -- the canonical, single-individual-climbing
P3 engine (Goldman 2014, Algorithm 3/4 shape): `first_improvement_hill_climber` (FIHC) + `climb` (FIHC then a
GOM sweep up through existing pyramid levels, promoting on improvement, growing a new level only once an
individual climbs past every existing one -- genuinely parameter-less, no stall counter or target level size).
Reuses `linkage_tree.py`/`optimal_mixing.py` unmodified, confirmed by a direct spy test. Reconstructed, not
transcribed -- neither Bartnik's nor Dushatskiy's algorithm listing is available in this repo; the module
docstring states this plainly. 9 new tests, `lib/tests/test_canonical_pyramid.py`.

**Faza 2**: `p3net.surrogates.absolute_random_forest.AbsoluteRandomForestSurrogate` (library) -- deterministic,
per-objective Random Forest absolute surrogate: two independent `sklearn.RandomForestRegressor` models (f1,
f2), sharing `_encoding.py`'s one-hot encoding with the existing linear `AbsoluteRegressorSurrogate`. A
dedicated XOR-pattern test confirms it captures a nonlinear interaction the linear surrogate cannot. No
uncertainty/`predict_std` API -- probabilistic RF is explicitly out of v1 scope (plan's Faza 5). 7 new tests,
`lib/tests/test_absolute_random_forest.py`.

**Faza 3**: `methods/bartnik_p3.py::BartnikP3` -- combines both library pieces with a lambda-quantile
acceptance gate (Dushatskiy 2021, Algorithm 7/8 shape) and an ell-random warm-up (ell = search space
dimensionality, 6 for architecture-only NAS-Bench-201). Key architectural decision documented in the module's
own docstring: the pyramid is climbed entirely against the surrogate's PREDICTED objectives (FIHC/GOM need a
fitness call at every step -- spending a real evaluation at each one would defeat a surrogate-assisted
algorithm's whole point); real evaluations are reserved for whatever a full climb's final candidate turns out
to be, gated by the lambda-quantile rule, the same "surrogate screens, real evaluation confirms" split
`p3net.methods.p3net.P3Net` already uses for its own (different) surrogate. Two real bugs found and fixed
during TDD: (1) `_register_fallback`'s gate relaxation multiplied tau by eta<1, which only relaxes the gate
when tau is positive -- for the common negative-tau case this made the gate stricter, backwards; fixed to
subtract a decaying, sign-agnostic margin so tau always decreases. (2) a reused-engine spy test patched the
wrong name (the library module, not `methods.bartnik_p3`'s own already-bound import) and silently passed with
zero calls recorded -- fixed to patch `methods.bartnik_p3.climb`. A documented performance characteristic
(module docstring): level 0's population never shrinks, so per-run cost grows faster than linearly in budget
on landscapes where most candidate pairs are mutually Pareto-nondominated (common when f2 takes few distinct
values, e.g. FLOPs) -- real, not mitigated in v1, surfaced rather than hidden. 9 new tests,
`tests/test_bartnik_p3.py`.

**Faza 4**: `configs/methods/bartnik_p3.yaml` (`default_grid: false` -- a genuinely separate analysis axis,
same convention as `configs/search_spaces/nas_bench_201.yaml`); wired into
`scripts/run_experiment.py::build_method`'s if-chain and its `parametrize`d wiring test
(`tests/test_run_experiment.py`); new `scripts/run_bartnik_isolation.py`, mirroring
`run_nas_bench_201_isolation.py`'s smoke test -> full grid -> Wilcoxon+Holm+Cliff's-delta pipeline, scoped to
`bartnik_p3` vs. the nine-method baseline set above, R=30 seeds x {100, 350} budgets on `nas_bench_201`.

**Smoke test against real data -- done.** `run_bartnik_isolation.py`'s own smoke test (budget=12, seed=1,
real NATS-Bench query via the already-downloaded `data/cache/nats_bench/` archive) passed end-to-end.

**Verification**: `uv run pytest` green in both `implementation/lib` (136 passed) and
`implementation/experiments` (239 passed, 2 skipped -- the usual opt-in-only live-JAHS-Bench-201 skips,
unrelated to this change) after this change.

**Not run by this change (manual, per this project's own established convention)**: the full grid --
`uv run python scripts/run_bartnik_isolation.py` from `implementation/experiments` -- R=30 seeds x {100, 350}
budgets x 10 methods = 600 real NATS-Bench queries.
