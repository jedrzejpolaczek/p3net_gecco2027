...
each benchmark's real data). The three benchmarks deliberately do **not**
share a genotype — see below — even though all three build on
`p3net.problem.genotype.SearchSpace`. The third, NAS-Bench-201
(architecture-only), was added for a single-axis isolation experiment,
not as a third arm of the main comparison grid — its search-space config
carries `default_grid: false` and is excluded from a plain `run_grid.py`
invocation.
...
C4Component
  ...
  System_Ext(natsbench, "nats_bench", "Real, in-process")

  Container_Boundary(search_spaces, "search_spaces") {
    Component(cell_graph, "_cell_graph.py", "has_input_output_path", "Six-edge DAG topology + path-existence check, shared by all three genotype modules")
    ...
    Component(nas_bench_201_genotype, "nas_bench_201_genotype.py", "nas_bench_201_search_space, nas_bench_201_validity, genotype_to_arch_str", "Architecture-only NAS-Bench-201: 6 edges, no Theta (isolation experiment)")
  }
  ...
  Container_Boundary(substrates, "substrates") {
    ...
    Component(natsbench_substrate, "nas_bench_201.py", "NASBench201Substrate", "In-process nats_bench queries, single fidelity level, f2=FLOPs")
  }

  Rel(nas_genotype, cell_graph, "path-existence validity check")
  Rel(nas_hpo_genotype, cell_graph, "path-existence validity check")
  Rel(nas_bench_201_genotype, cell_graph, "path-existence validity check")
  ...
  Rel(nas_bench_201_genotype, p3net_problem, "SearchSpace/Genotype/Validity")
  Rel(jahs_substrate, base, "implements Substrate")
  Rel(nashpo_substrate, base, "implements Substrate")
  Rel(natsbench_substrate, base, "implements Substrate")
  ...
  Rel(natsbench_substrate, nas_bench_201_genotype, "decodes genotypes it's queried with")
  Rel(jahs_substrate, jahsbench_bridge, "JSON-lines over stdin/stdout")
  Rel(nashpo_substrate, nashpobench, "In-process call")
  Rel(natsbench_substrate, natsbench, "In-process call")
```
...
| **nas_bench_201_genotype.py** | `nas_bench_201_search_space()`: architecture-only NAS-Bench-201 genotype (6 edges, no Θ at all) — built for the isolation experiment testing whether this project's own joint architecture+hyperparameter extension is behind P3Net's weak results (`conclusions/main.tex`, "Why the results are what they are"); `genotype_to_arch_str` translates to `nats_bench`'s own lookup format | `search_spaces/nas_bench_201_genotype.py` |
...
| **nas_bench_201.py** | `NASBench201Substrate`: real, in-process queries via `nats_bench`; single fidelity level (200 epochs, deliberately no ladder); `f2` = FLOPs, a documented deviation from Bartnik's own measured GPU energy (not a queryable field of this benchmark) | `substrates/nas_bench_201.py` |
