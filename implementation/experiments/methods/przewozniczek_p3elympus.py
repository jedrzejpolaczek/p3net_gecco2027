...
class PrzewozniczekP3ELyMPuS:
    ...
    #: Local-search step inside each climb. "fihc_elympus" (default) is the
    #: arm itself. "fihc_scalar" is the B4 control arm
    #: (configs/methods/canonical_p3_scalar_fihc.yaml): plain first-
    #: improvement hill climbing on the SAME scalar objective (the forest's
    #: predicted f1) that eLyMPuS compares, with the pyramid, acceptance gate
    #: and forest unchanged. bartnik_p3's default climber differs from this
    #: arm in two ways at once -- the comparison mechanism AND permissive
    #: multi-objective acceptance -- so only this control isolates eLyMPuS
    #: (review finding B4).
    hill_climber: str = "fihc_elympus"
    ...
    #: Uniform sample of surrogate decisions (gate and hill climber), checked
    #: against the benchmark after the run; never influences the search.
    ...
    def __post_init__(self) -> None:
        if self.hill_climber not in ("fihc_elympus", "fihc_scalar"):
    ...
    def update(self, state: RunState, new_observations: list[Observation]) -> None:
        ...
        for obs in new_observations:
            best_before = min(
                (
                    o.objectives[self.objective_index]
    ...
    def _gated_climb_proposal(self) -> list[Genotype]:
        ...
        best_real = min(
            self._history, key=lambda g: self._history[g].objectives[self.objective_index]
        )
        best_real_f1 = self._history[best_real].objectives[self.objective_index]
        ...
        if self.hill_climber == "fihc_scalar":
            ...
            elympus = ELyMPuS(
                ...
                fitness_fn=make_elympus_fitness_adapter(
                    surrogate.predict, objective_index=self.objective_index
                ),
        ...
        for _ in range(self.max_climb_attempts):
            ...
            self.decision_log.improvement(
                ...
                threshold=self._tau,
                accepted=predicted_delta >= self._tau,
            )
