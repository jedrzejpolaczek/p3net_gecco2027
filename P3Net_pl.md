...
Ten schemat kontynuuje wzorzec, który White i in. \citep{white2023insights} wskazują jako najpopularniejsze podejście w wielokryterialnym NAS: ewolucję architektur za pomocą wielokryterialnego algorytmu ewolucyjnego, czego przykładem jest tam wprost przywoływany NSGANetV2. Liu i in. \citep{liu2023survey} wymieniają połączenie tego silnika z wyuczonym predyktorem zastępczym jako jedną z kilku uznanych technik redukcji kosztu ewaluacji, obok współdzielenia wag i tanich (zero-cost) predyktorów; żaden z tych dwóch przeglądów nie wskazuje tej kombinacji jako jedynego dominującego paradygmatu dziedziny.
...
Zela i in. \citep{zela2018towards} podobnie optymalizują wspólnie kategoryczne metaparametry wielogałęziowej architektury ResNet: liczbę bloków resztkowych i gałęzi na blok oraz współczynnik poszerzenia na blok. Zmienne te są przeszukiwane razem z siedmioma ciągłymi hiperparametrami treningowymi: początkowym współczynnikiem uczenia, rozmiarem batcha, momentum, zanikiem wag L2, długością CutOut, parametrem $\alpha$ MixUp i częstością ShakeDrop. Liczba epok pełni tu wyłącznie rolę wymiaru zasobu w Hyperbandzie, nie zmiennej decyzyjnej.

P3Net stosuje tę samą konwencję (Wprowadzenie). Bartnik \citep{bartnik2026evolutionary} omija tę kwestię całkowicie, bo jej genotyp nie ma wymiaru hiperparametrów: procedurę treningową ustala tablica NAS-Bench-201, a jej dyskusja przyznaje, że wnioski opisują tę zamkniętą przestrzeń, nie NAS w ogólności. To ograniczenie motywuje ewaluację na rzeczywiście wspólnych benchmarkach, takich jak wymienione powyżej.

Przeanalizowana literatura różni się też źródłem wyniku, który benchmark uznaje za prawdziwy przy ewaluacji kandydata. Zela i in. \citep{zela2018towards} optymalizują ustawienie wspólne przez BOHB na rzeczywistych przebiegach treningowych. NAS-HPO-Bench-II \citep{hirose2021nashpobenchii} odpowiada na każde zapytanie przez stałą tablicę zmierzonych wartości, a JAHS-Bench-201 \citep{bansal2022jahsbench} przez wyuczony predyktor zastępczy; żaden z nich nie trenuje niczego w chwili ewaluacji. Benchmarki pierwszego typu udostępniają wyczerpującą tablicę wszystkich kombinacji, nad którą można obliczyć dokładny front Pareto; nazywamy je dalej *Kategorią 1*. Benchmarki drugiego typu odpowiadają przez wyuczony, ciągły predyktor zastępczy i nie mają enumerowalnego frontu oracle; nazywamy je *Kategorią 2*. W tej terminologii NAS-HPO-Bench-II należy do Kategorii 1, a JAHS-Bench-201 do Kategorii 2.

Klasyfikacja NAS-HPO-Bench-II jako Kategorii 1 zakłada, że P3Net odpytuje wyłącznie jego wyczerpująco stablicowany zakres: do 12 epok treningu, z trzema zarejestrowanymi ziarnami na wpis. Sam benchmark udostępnia też osobny, wyuczony surogat (sieć GIN połączona z MLP), który ekstrapoluje wyniki do 200 epok. Gdyby ta ekstrapolacja stanowiła poziom pełnej ewaluacji $r_K$, podważyłoby to zarówno klasyfikację Kategorii 1, jak i status dokładnego frontu oracle. Tak się jednak nie dzieje: poziom $r_K$ P3Net dla tego benchmarku jest ustalony wyłącznie na stablicowanym zakresie, a 200-epokowa ekstrapolacja surogatu nigdy nie jest odpytywana przez pętlę przeszukiwania. Klasyfikacja Kategorii 1 i status frontu oracle są więc zachowane.

Główne benchmarki P3Net to właśnie te dwa, wybrane z powodów już ustalonych w tym nurcie i szerzej w literaturze NAS Kategorii 1: deterministyczna funkcja celu, obliczalny front oracle, ograniczony budżet ewaluacji.

Własny surogat JAHS-Bench-201 nie powinien być mylony z wyuczonym predyktorem zastępczym, który podczas przeszukiwania budują P3Net, CS-GOMEA \citep{dushatskiy2019csgomea}, Dushatskiy i in. \citep{dushatskiy2021novelsurrogateassistedevolutionaryalgorithm} oraz LyMPuS \citep{przewozniczek2026lympus}. Surogat JAHS-Bench-201 zastępuje funkcję celu raz, offline, jeszcze przed rozpoczęciem przeszukiwania. Predyktor zastępczy tych metod uczących powiązania działa inaczej: jest dopasowywany przyrostowo w trakcie przeszukiwania, wyłącznie po to, by decydować, którzy kandydaci zasługują na pełne zapytanie. To dwie niezależne decyzje projektowe na dwóch różnych poziomach; wkład P3Net dotyczy wyłącznie tej drugiej, predyktora działającego w czasie przeszukiwania, nałożonego na dowolne podłoże odpowiadające za pełną ewaluację.

Wśród prac łączących taki predyktor działający w czasie przeszukiwania z algorytmem uczącym powiązania, najbliższa niniejszej pracy jest praca Bartnik \citep{bartnik2026evolutionary}. Adaptuje ona SA-P3-GOMEA (algorytm uczący powiązania wspomagany predyktorem zastępczym) do wielokryterialnego NAS na NAS-Bench-201, optymalizując dokładność walidacyjną względem zmierzonego zużycia energii GPU. Zastosowany tam predyktor jest bezwzględnym regresorem nad płaskim genotypem (deterministyczne lub probabilistyczne warianty SVR, MLP, lasu losowego, gradient boostingu). Metodami odniesienia są MO-LS i pozbawiony predyktora MO-P3-GOMEA, nie NSGA-II czy NSGANetV2. Przestrzeń przeszukiwania to zamknięty benchmark NAS-Bench-201 ograniczony do architektury, bez hiperparametrów treningowych w genotypie.

Wychodząc poza szczegóły samej pracy Bartnik \citep{bartnik2026evolutionary}, szersza literatura przeglądowa potwierdza marginalną pozycję tej rodziny algorytmów: trzy niezależne systematyczne przeglądy ewolucyjnego NAS \citep{liu2023survey, white2023insights, ozcelik2026survey} nie wymieniają GOMEA, P3 ani uczenia powiązań jako odrębnej kategorii metod, co wskazuje, że ta rodzina pozostaje poza głównym nurtem dziedziny. Połączenie ewolucyjnego algorytmu uczącego powiązania z wyuczonym predyktorem zostało już zweryfikowane empirycznie, a jak opisano powyżej, praca Bartnik \citep{bartnik2026evolutionary} jako pierwsza rozszerza je na NAS. Cztery aspekty pozostają jednak wspólnie nierozwiązane: relatywny predyktor zastępczy świadomy struktury powiązań zamiast bezwzględnego regresora nad płaskim genotypem; NSGA-II/NSGANetV2 jako bezpośrednio porównywana metoda odniesienia; wspólne przeszukiwanie architektury i hiperparametrów, do którego, zgodnie z naszą wiedzą, żaden ewolucyjny algorytm uczący powiązania nie był wcześniej stosowany; oraz wpływ świadomości powiązań na wskaźnik duplikatów genotypu w przestrzeni typu graf-komórka. Nawet Bartnik \citep{bartnik2026evolutionary}, jedyna dotychczasowa praca z tej linii operująca na takiej przestrzeni (NAS-Bench-201, podatnej na ten sam izomorfizm co omówiony we Wprowadzeniu JAHS-Bench-201), nie raportuje tego wskaźnika jako odrębnej diagnostyki.
...
*(TODO: zaraportować tutaj wyniki, w kolejności zapowiedzianej na początku sekcji: jakość predyktora zastępczego
(korelacja rangowa w funkcji $|\mathcal{H}_t|$); zbieżność przy każdym ustalonym budżecie ewaluacji; porównania
frontu Pareto i hiperwolumenu dla wszystkich dziewięciu baseline'ów/ablacji na obu benchmarkach; oraz zbiorczą
tabelę dla ustalonego budżetu (mediana, IQR, skorygowane wartości $p$, wielkości efektu) z Planu statystycznego.
Dołączyć analizę wrażliwości $\kappa$/progu akceptacji oraz diagnostykę wskaźnika duplikacji, gdy zostaną
przeprowadzone.)*
...
- Ograniczenia: jakość predyktora zastępczego zależy od jakości kodowania genotypu; drzewo powiązań budowane jest na podstawie informacji wzajemnej między parami zmiennych, co może być zbyt grubym sygnałem, by uchwycić wyższego rzędu, głęboko ustrukturyzowane zależności. JAHS-Bench-201 odpowiada na każde zapytanie przez wyuczony surogat, nie przez zmierzone dane, więc wyniki na nim odziedziczają błąd aproksymacji tego surogatu względem realnego, niezależnego treningu; ponieważ wszystkie porównywane metody odpytują ten sam surogat, obciążenie systematyczne wpływa na nie równo, ale nie wyklucza to korelacji tego błędu akurat ze strukturą wykorzystywaną przez P3Net. Wynik utrzymujący się także na NAS-HPO-Bench-II, opartym na zmierzonych danych, jest dlatego traktowany jako główne odkrycie, a wynik specyficzny wyłącznie dla JAHS-Bench-201 jako wymagający potwierdzenia.
...
| Recenzent zewnętrzny zgłosił „bałagan” w cytowaniu / „niezgodnie ze sztuką” | Naprawione | Usunięto jedno zbędne, powtórzone w tym samym akapicie cytowanie tego samego źródła (piramida P3, \citep{goldman2014parameterless}); w wersji LaTeX ujednolicono spację przed każdym cytowaniem na twardą (`~\citep{}` zamiast zwykłej spacji — 33 przypadki groziły osieroceniem numeru cytowania na początku linii); ujednolicono 4 przypadki gołego `\cite{}` do `\citep{}`, zgodnie z resztą pracy, która nigdzie nie używa `\citet{}` | Prace pokrewne; Wyniki |
| Skutek uboczny powyższej poprawki spacji: automatyczna podmiana kolidowała z istniejącym makrem `\ ` (spacja kontrolna) po „et al.”, psując 5 miejsc do `\~\citep{}` (nadmiarowy backslash przed tyldą, który LaTeX odczytałby jako polecenie akcentu-tyldy, nie jako spację) | Naprawione | Wychwycone przy audycie cytowań pod eksport do Worda; poprawiono wszystkie 5 miejsc (`et al.\ \citep` → `et al.~\citep`) | Wprowadzenie; Prace pokrewne |
| Recenzentka zewnętrzna (przez zaanotowany PDF, `NAS_com_v1.pdf`) zgłosiła trzy kolejne problemy z cytowaniami | Naprawione | (1) Pierwsze dwa zdania Wprowadzenia (dominujący koszt NAS; sekwencyjne strojenie architektury/hiperparametrów jako powszechna praktyka) nie miały żadnego źródła przed pierwszym cytowaniem trzy zdania dalej; dodano odpowiednio `\citep{white2023insights,liu2023survey}` i `\citep{lu2019nsganet,lu2020nsganetv2}`. (2) Trzy miejsca wypisywały wszystkich autorów pracy z 3+ autorami zamiast „i in.” (Whitley/Chicano/Goldman; Dushatskiy/Alderliesten/Bosman; Tran/Truong/Vo/Luong); skrócono do „i in.” zgodnie z konwencją stosowaną w reszcie pracy. (3) Akapit z analizą luki badawczej wymieniał „Bartnik” trzykrotnie bez żadnego cytowania przy żadnym z tych wystąpień (najbliższe `\citep{bartnik2026evolutionary}` było dwa akapity wcześniej); dodano cytowanie przy każdym gołym wystąpieniu | Wprowadzenie; Prace pokrewne |
